...
def agent_main():
    new_input_messages = [
        {"role": "user", "content": "查下北京的天气，然后给foo@bar.com发一封邮件告知内容"}
    ]

    response = client.responses.create(
        model="gpt-4.1", input=new_input_messages, tools=TOOL_CONFIG
    )

    for step in response.output:
        new_input_messages.append(step)
        result = FUNCTION_MAP[step.name](
            **json.loads(step.arguments)
        )  # 通过字典数据结构实现了一种“反射”的设计模式。便于扩展和debug
    ...
    for step in response_2.output:
        new_input_messages.append(step)
        result = FUNCTION_MAP[step.name](**json.loads(step.arguments))
        new_input_messages.append(
